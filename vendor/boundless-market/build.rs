...
fn sorted_sol_files(dir: &Path) -> Vec<std::path::PathBuf> {
    let mut paths: Vec<_> = fs::read_dir(dir)
        .unwrap()
        .filter_map(|entry| {
            let path = entry.ok()?.path();
            (path.extension().and_then(|s| s.to_str()) == Some("sol")).then_some(path)
        })
        .collect();
...
fn rewrite_solidity_interface_files() {
    ...
    let sol_iface_dir = Path::new(&manifest_dir)
        .join("src")
        .join("contracts")
        .join("artifacts");
    println!(
        "cargo::rerun-if-changed={}",
        sol_iface_dir.to_string_lossy()
    );
    ...
    for path in sorted_sol_files(&sol_iface_dir) {
        ...
        while let Some(start) = sol_contents.find("library ") {
            if let Some(end) = find_matching_brace(&sol_contents[start..]) {
                ...
                panic!("Unmatched brace in library {:?}", path.file_name().unwrap());
            }
        }
