...
mod route_tests {
    ...
    #[test]
    fn tdx_remote_route_uses_tdx_pipeline() {
        let route = PipelineRoute::new(GuestSystem::Tdx, RunnerKind::Remote);
        assert_eq!(
            route.pipeline_key().expect("supported route"),
        ...
        assert_eq!(route.proof_type(), raiko2_primitives::ProofType::Tdx);
    }
