...
enforce checked-in ELF freshness. A stable required check should therefore exist in the main CI
workflow and always report one of two states:
...
- Add a new `guest-elf-consistency` job to `ci.yml`.
...
- Do not rework the existing guest build artifact jobs in this change.
