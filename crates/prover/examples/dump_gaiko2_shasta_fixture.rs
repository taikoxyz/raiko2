...
fn main() {
    ...
    let packet = if include_guest_input {
        build_shasta_packet_with_guest_input(&guest_input).expect("adapt guest input shasta packet")
    } else {
        build_shasta_packet(&guest_input).expect("adapt guest input shasta packet")
    };
    ...
    fs::write(&output_path, serialized).expect("write shasta packet fixture");
}
