...
impl<F: HardforkSpec> crate::Prover<F> for Risc0Prover {
    async fn prove(
        &self,
        input: GuestInput,
        config: &ProverConfig,
        ...
        let elf = spec.elf(ProverBackend::Risc0, ProofStage::Proposal)?;
    ...
    async fn aggregate(
        ...
        let batch_elf = spec.elf(ProverBackend::Risc0, ProofStage::Proposal)?;
        ...
        let elf = spec.elf(ProverBackend::Risc0, ProofStage::Aggregation)?;
        let image_id = compute_image_id(elf)
            .map_err(|e| RaikoError::Guest(format!("Failed to compute image id: {}", e)))?;
