...
This harness targets replay-packet remote providers such as `gaiko2`/`sgxgeth`. It is not a
`raiko2-sgx-prover` smoke test; that provider requires a complete `GuestInput` envelope and runs
the Shasta guest validation path before signing.
