...
for PROOF_TYPE in $PROOF_TYPES; do
  "$PYTHON" "$REPO_ROOT/scripts/regression/stress_shasta_proposal.py" \
    ...
    --l1-rpc "$L1_RPC" --l2-rpc "$L2_RPC" \
    --raiko-rpc "$RAIKO_RPC" --proposal-ids "$PROPOSAL_IDS" \
    --prove-type "$PROOF_TYPE" --api-version v4 \
  ...
  "$PYTHON" "$SKILL_ROOT/scripts/v4_aggregate.py" \
    ...
    --proof-type "$PROOF_TYPE" --poll-interval 5 2>&1 \
...
the script default is v3.

## Acceptance

Pass only when every requested base proposal and aggregate reaches `completed`. Report proposal
tuples, proof type, task ID, final status, proof byte length, duration, and any host/remote errors.
