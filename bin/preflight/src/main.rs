...
use clap::Parser;
use raiko2_pipeline::proposal::{ProposalSpec, preflight_cache::PreflightCoordinator};
...
async fn main() -> Result<()> {
    ...
    ctx.preflight.resolved_l2_chain_spec = Some(resolved.l2_chain_spec.clone());
    ...
    ctx.preflight.verify_checkpoint_l2_rpc = args
        ...
        .map(str::to_owned);
    let spec = ProposalSpec::new(PipelineKey::NativeLocal, (), NativeBackend, provider)
