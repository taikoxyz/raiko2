...
- **Objective:** Make proof publication, invalidation, recovery, ownership, and readiness converge on one durable state without allowing stale owners, dangling GCS objects, or observer failures to produce reusable invalid proofs or duplicate paid submissions.
- **Authority:** Runtime namespace ownership and the authoritative runtime state generation gate every shared-store mutation; engine state is execution state, not the durable source of truth for API recovery.
...
- R2. Invalidation must fence namespace ownership before every shared-store mutation and invalidate only the selected publication generation.
...
- R7. Runtime ownership heartbeat starts immediately after ownership acquisition and is stopped if initialization fails.
- R8. `/ready` fails when runtime ownership is inactive, the authoritative runtime store is inaccessible, or queue maintenance is not demonstrably live.
...
- AE5. Given namespace ownership is lost during startup or steady state, shared mutations stop and `/ready` reports an error before more work is accepted.
...
In scope are the seven P2 Kimi findings plus startup heartbeat timing, the cancel-after-commit race, real queue readiness, and the GCS/recovery tests needed to prove those fixes.
...
- KTD6. **Persist remote submission progress synchronously.** `ProverProgressObserver` returns an error, and a newly submitted request remains attached to the current stage lease while checkpoint persistence retries. If ownership is lost, the worker stops; it never converts a failed checkpoint into permission to submit again in the same process.
...
- KTD8. **Start lease renewal under a startup guard.** Ownership acquisition creates a cancellable heartbeat guard before initialization, fencing, artifact restoration, or task recovery. Failed startup cancels and joins the heartbeat.
...
```mermaid
flowchart TB
  Probe[/ready] --> Ownership{Runtime owner active and store reachable?}
  Ownership -->|no| NotReady[Not ready]
  Ownership -->|yes| Queue{Recent successful queue maintenance?}
  Queue -->|no| NotReady
...
- **Persistence:** Runtime state, GCS manifests, invalidation markers, and the pending publication outbox must agree across owner failover.
...
- **Test scenarios:**
  ...
  5. A stale owner or stale generation cannot create an invalidation marker or delete the newer manifest.
...
- **Approach:** Carry a generation-qualified publication descriptor through observer completion. Combine root success and artifact registration in one authoritative runtime mutation, clear the pending outbox only after that mutation, and retain the descriptor when synchronization fails. Cancellation or a detached root invalidates the exact committed generation even when the pending outbox was already drained.
...
- **Test scenarios:**
  ...
  5. Owner loss at each external mutation boundary prevents GCS or runtime-state changes by the stale instance.
...
### U4. Ownership-safe startup and real readiness

- **Goal:** Renew ownership throughout startup and reject traffic whenever runtime or queue mutation capability is not healthy.
...
- **Approach:** Add a cancellable heartbeat guard immediately after owner acquisition. Track queue maintenance last-success time and last error in shared application state without mutating queue contents from the probe. Readiness combines runtime ownership/store readiness with a freshness threshold derived from the configured maintenance interval.
- **Test scenarios:**
  1. Covers AE5. Slow restoration longer than one lease remains owned because heartbeat renewal starts before restoration.
  2. Initialization failure cancels the heartbeat and does not leave a background owner renewal task.
  3. Ownership loss or authoritative store failure makes runtime readiness fail.
...
- **Verification:** Readiness tests observe real shared health state, and startup tests prove heartbeat lifecycle cleanup on both success and failure.
...
- **Approach:** Change progress observation to return a persistence result across all prover backends. After SP1 submission, retry the same checkpoint while holding the stage lease and request ID; propagate owner loss without issuing another request. Normalize zero deadlines to absent and use the configured full timeout for legacy resume while preserving request ID and attempt.
...
- **Test scenarios:**
  ...
  2. Permanent ownership loss stops the stage and records an explicit checkpoint failure without same-process resubmission.
...
- **Approach:** State that active manifests have no independent age-based deletion, immutable content must outlive every referencing manifest generation, and invalidation markers must outlive the maximum retry/recovery window for their generation. Add an operator-visible note for legacy markers and dangling-object repair. Run the integrated restart, ownership-loss, cancellation, deterministic-reproof, and remote-checkpoint scenarios.
- **Test scenarios:**
  ...
  2. Owner failover during invalidation prevents the old owner from mutating the new generation.
  3. Deterministic reproof after invalidation is reusable through the API.
  4. Readiness transitions to error on owner/queue failure and returns to ready after successful recovery.
...
| `cargo test -p raiko2-runtime -p raiko2-queue -p raiko2-engine` | U1-U4, U7 | Storage, reconciliation, ownership, and queue health scenarios pass. |
...
- A stale namespace owner cannot write runtime state, manifests, deletions, or invalidation markers.
