...
use raiko2_hardfork::{HardforkSpec, ProofStage, ProverBackend};
...
impl<F: HardforkSpec> crate::Prover<F> for Sp1Prover {
    async fn prove(
        &self,
        input: GuestInput,
        config: &ProverConfig,
        ...
        let elf = spec.elf(ProverBackend::Sp1, ProofStage::Proposal)?;
    ...
    async fn aggregate(
        ...
        let batch_elf = spec.elf(ProverBackend::Sp1, ProofStage::Proposal)?;
        let client = ProverClient::from_env();
        ...
        let elf = spec.elf(ProverBackend::Sp1, ProofStage::Aggregation)?;
