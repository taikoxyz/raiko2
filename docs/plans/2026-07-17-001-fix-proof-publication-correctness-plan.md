...
- R10. Provider checkpoints require a non-zero attempt and deadline plus all backend-specific fields; malformed or obsolete records fail closed.
...
- AE6. Given an SP1 request ID is returned but checkpoint persistence fails transiently, the same worker retries the checkpoint without submitting another request; malformed checkpoints are rejected.
...
- KTD10. **Use one global namespace fence, never task-local authority.** (session-settled: user-directed — chosen over per-task owner epochs: the deployment model has exactly one live process per namespace.) Draining fences new work and all ordinary task, publication, invalidation, reconciliation, and cleanup writes together. Only a remote request admitted before the drain may persist its own request-ID checkpoint through its runtime-issued permit; inactive fences that write too. Immutable task incarnations and lease tokens may reject stale local callbacks but never grant write authority. GCS generations remain independent object-version CAS tokens.
...
- **Persistence schema cutover:** Existing content-hash-only markers and legacy runtime snapshots are unsupported. Deploy with a new empty namespace and fail closed on legacy state; do not add lazy migration or compatibility fallback.
...
### U5. Durable SP1 submission checkpoints and strict resume

- **Goal:** Make returned SP1 request IDs durably resumable before normal waiting continues and reject incomplete or incompatible checkpoint records.
...
- **Approach:** Change progress observation to return a persistence result across all prover backends. After SP1 submission, retry the same checkpoint while holding the stage lease and request ID; propagate permanent global lifecycle failure without issuing another request. Reject zero deadlines and incomplete checkpoint payloads.
...
- **Test scenarios:**
  ...
  3. A zero-deadline or incomplete checkpoint is rejected without hidden fallback.
...
- **Approach:** State that active manifests have no independent age-based deletion, immutable content must outlive every referencing manifest generation, and invalidation markers must outlive the maximum retry/recovery window for their generation. Document the new-namespace hard cut and dangling-object repair. Run the integrated restart, lifecycle-fence, cancellation, deterministic-reproof, and remote-checkpoint scenarios.
- **Test scenarios:**
...
| `cargo test -p raiko2-prover` | U5-U6 | Strict submission checkpoint, provider-config identity, and retry-cap scenarios pass. |
| `cargo test -p raiko2` | U2-U7 | Server observer, API recovery, startup, and readiness integration scenarios pass. |
| `cargo clippy --workspace -- -D warnings` | Final integration | No new warnings across workspace library and binary targets. |
| `git diff --check` | Final integration | No whitespace or patch integrity errors. |
| `just build-guest sp1` and `just build-guest risc0` | Prover backend integration | Guest artifacts are current and host/guest contracts still build. |

The guest build gates remain required because the change touches prover backend code, even though it does not intentionally change guest sources or proof formats.
...
- A draining runtime permits only the request-ID checkpoint of a remote request admitted before the drain; it rejects every other runtime-state, manifest, checkpoint, deletion, and invalidation write. Inactive rejects all writes.
...
- SP1 checkpoint failures do not trigger same-process duplicate submissions, malformed checkpoints fail closed, and client overrides cannot exceed the operator attempt cap.
