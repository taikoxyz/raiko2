//! RISC0 guest program for Boundless-backed aggregation proofs.
#![no_main]
#![allow(missing_docs)]
...
use raiko2_primitives_shasta::{
    ShastaBoundlessAggregationGuestInput, ShastaZkAggregationGuestInput, instance::words_to_bytes_le,
};
...
pub fn main() {
    ...
    let input: ShastaBoundlessAggregationGuestInput =
        bincode::deserialize(&input_buf).expect("failed to deserialize boundless aggregation input");
    ...
    let image_id =
        Digest::try_from(image_id_bytes.as_slice()).expect("invalid boundless aggregation image id");

    let block_inputs = input
        .receipts
        .iter()
        .map(|encoded_receipt| {
            let receipt: Receipt = bincode::deserialize(encoded_receipt)
                .expect("failed to deserialize boundless receipt");
            receipt
                .verify(image_id)
