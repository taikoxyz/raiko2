...
impl crate::Prover for Sp1Prover {
    ...
    async fn aggregate(
        ...
        let batch_elf = elf::SP1_BATCH_ELF;
        let client = ProverClient::from_env();
        ...
        for proof in &input.proofs {
            ...
            if let Some(proof_hex) = &proof.proof {
                let proof_bytes = alloy_primitives::hex::decode(proof_hex).map_err(|e| {
                    ProverError::GuestError(format!("Failed to decode proof hex: {}", e))
                })?;
        ...
        let elf = elf::SP1_SHASTA_AGGREGATION_ELF;
        ...
        let proof = client
            ...
            .map_err(|e| {
                ...
                ProverError::GuestError(format!("SP1 aggregation proof generation failed: {}", e))
            })?;
        ...
        if self.config.verify {
            client.verify(&proof, &vk).map_err(|e| {
                ...
                ProverError::GuestError(format!("SP1 aggregation proof verification failed: {}", e))
            })?;
