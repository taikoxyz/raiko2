...
Final-attempt terminalization must also be confirmed by one canonical chain snapshot. Host
wall-clock time may trigger ordinary rebids, but it cannot prove that a request's payable window has
closed. The status source first fetches the latest header, then pins every market `eth_call` to that
header's block hash with `requireCanonical = true`. The final no-lock checkpoint is cleared only
when the pinned market state is unlocked and that same block's timestamp is strictly greater than
the request's lock deadline, matching the Boundless market pricing boundary. At the exact deadline
the request remains payable. If the header or pinned state is unavailable or reorged, the final
attempt remains pending and retains its checkpoint.
