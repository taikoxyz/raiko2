...
does not select child counts or compare the running guest image. The aggregation ELF recorded by
this calibration artifact has not been measured successfully. Its SHA-256 is
...
artifact provenance therefore has `image_id: null`. The current checked-in aggregation guest no
longer hashes to that value. Historical
...
The proposal measurements predate the current checked-in proposal ELF and do not use the v0.6.0
